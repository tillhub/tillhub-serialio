...
class SerialIO {
   ...
   */
  constructor (port) {
    debug('initializing SerialIO on port:', port)
    this._sp = sp
    ...
    this._port.on('error', (err) => {
      debug('SerialIO error event:', err)
    })
    this._port.on('drain', (err) => {
      debug('SerialIO drain event:', err)
    })
  ...
  closeHandler (err) {
    if (!this._closing) {
      debug('unexpected closing of port!', err)
      debug('trying to reopen port...')
      ...
      debug('not reopening after expected close')
    }
  }
  ...
  _handleMessage (rawMsg, type) {
    try {
      ...
      debug(`${this._portString} > [${type}]: ${rawMsg.substring(0, 120)}… SIZE: ${rawMsg.length} BYTES`)

      let parsedMsg
      try {
        parsedMsg = JSON.parse(rawMsg)
      } catch (e) {
        debug('cannot parse message to JSON')
      }
      ...
      if (type >= SerialIO.MESSAGE_TYPE.REPLY) {
        if (this._replyPromise) {
          ...
          try {
            if (type === SerialIO.MESSAGE_TYPE.REPLY) { this._replyPromise.resolve(parsedMsg || rawMsg) } else if (type === SerialIO.MESSAGE_TYPE.ERROR) { this._replyPromise.reject(rawMsg) }
          } catch (e) {
            debug('Error while resolving reply promise', e)
          } finally {
        ...
        if (this._msgHandler) {
          (async () => {
            try {
              ...
              try {
                ...
                debug('sending reply failed', e)
              }
            } catch (e) {
              debug('error while calling message handler')
              this.sendErrorReply(e).catch((err) => {
                debug('sending message handler error as reply failed', err)
              })
            }
          })()
        } else {
          ...
          this.sendErrorReply('No message handler to handle message').catch((err) => {
            debug('sending missing message handler error as reply failed', err)
          })
        }
      }
    } catch (e) {
      debug('Unable to handle data:', e)
  ...
  _parseData (data, from = 0) {
    debug('_parseData', data.length, from, data.length - from)
    ...
    debug('_parseData', data.length, from, data.length - from, iStart)
    ...
    if (!this._buf || this._buf.used === this._buf.length) {
      ...
      debug(`created new ${msgLength}b buffer for message type ${msgType}, starting from data at index ${iStart}`)
    ...
    if (iStart > from && iStart <= from + Math.min(bufRemaining, dataRemaining)) {
      ...
      if (this._replyPromise) { this._replyPromise.reject('Message incomplete') }
    ...
    if (dataRemaining <= bufRemaining) { // data is smaller/equal rest of buffer
      ...
      if (iStart > from) {
        ...
        if (this._replyPromise) { this._replyPromise.reject('Message incomplete. Case #1') }
      ...
      if (this._buf.used === this._buf.length) {
        ...
        try {
          ...
          debug('Error while trying to handle message', e)
        }
      }
  ...
  send (msgBody = '', msgType = SerialIO.MESSAGE_TYPE.REQUEST) {
    debug(`send length: ${msgBody.length !== undefined ? msgBody.length : '?'}, typeof: ${typeof msgBody}, msgType: ${msgType}`)
    return new Promise((resolve, reject) => {
      if (this.sending) {
      ...
      let outString
      if (msgBody instanceof Error) { outString = msgBody.toString() } else if (typeof msgBody !== 'string') {
        try {
          outString = JSON.stringify(msgBody)
        } catch (e) {
          debug("msgBody is not a string, but can't be stringified by JSON")
      ...
      debug(`${this._portString} < [${msgType}] ${msgBody.constructor.name}: ${outString.substring(0, 120)}… SIZE: ${sBuf.length} BYTES`)
  ...
  _startReplyTimeout () {
    ...
    if (this._replyPromise) {
      this._replyTimeout = setTimeout(() => {
        if (this._replyPromise) { this._replyPromise.reject('Timeout reached') } else {
   ...
   */
  _clearReplyTimeout () {
    if (this._replyTimeout) { clearTimeout(this._replyTimeout) }
