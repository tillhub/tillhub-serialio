...
class SerialIO {
    ...
    this._port.on('error', (err) => {
      ...
      if (this._onErrorHandler) {
        try {
          ...
          debug('error handler returned with error: %s', e.message || e)
        }
      }
    })
    this._port.on('drain', (err) => {
      debug('drain event: %s', err)
      if (this._onDrainHandler) {
        try {
          this._onDrainHandler(err)
        } catch (e) {
          debug('drain handler returned with error: %s', e.message || e)
        }
      }
    })

    this._port.on('open', (err) => {
      debug('open event: %s', err)
      if (this._onOpenHandler) {
        try {
          this._onOpenHandler(err)
        } catch (e) {
          debug('open handler returned with error: %s', e.message || e)
        }
      }
    })
    // this.open();
  ...
  send (msgBody = '', msgType = SerialIO.MESSAGE_TYPE.REQUEST) {
    debug(`send %s message, type %s`, typeof msgBody, Utils.toHex(msgType))
  ...
  _parseData (data, from = 0) {
    const iStart = data.indexOf(Utils.START_SEQUENCE, from)
    ...
    if (this._tempBuf && from === 0) {
      debug(`prepending ${this._tempBuf.length}b of _tempBuf to data`)
    ...
    if (!this._buf || this._buf.used === this._buf.length) {
      ...
      debug(`creating new ${msgLength}b buffer for message type ${Utils.toHex(msgType)}, starting from data at index ${iStart}`)
    ...
    try {
      ...
      if (type >= SerialIO.MESSAGE_TYPE.REPLY) {
        ...
        if (this._onMessageHandler) {
          (async () => {
            try {
              let reply = this._onMessageHandler(parsedMsg || rawMsg)
  ...
  _closeHandler (err) {
    ...
    if (this._onCloseHandler) {
      try {
        ...
        debug('close handler returned with error: %s', err.message || err)
      }
    }
    if (!this._closing) {
      debug('unexpected closing of port: %s', err.message || err)
