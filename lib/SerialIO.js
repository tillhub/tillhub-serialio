...
class SerialIO {
  ...
  constructor(port) {
    ...
    const timoutCheckerId = setInterval(async () => {
  ...
  async _dataHandler(rawMsg) {
    try {
      ...
      console.log(`got data of length ${rawMsg.length}`);
      ...
      console.debug(`${this._portString} > #${pMsg.id}${pMsg.reply ? " (REPLY)" : ""} ${pMsg.constructor.name}: ${rawMsg.substring(0, 120)}…`);
  ...
  send(msg, awaitReply = true) {
    return new Promise((resolve, reject) => {
      ...
      console.debug(`${this._portString} < #${msg.id}${msg.reply ? "/R" : ""} ${msg.constructor.name}: ${outString.substring(0, 120)}…`);
