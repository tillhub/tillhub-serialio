...
class SerialIO {
          ...
          (async () => {
            try {
              ...
              if (reply instanceof Promise) {
                console.debug('message handler returned Promise. Awating reply...')
                reply = await reply
              }
  ...
  _parseData (data, from = 0) {
    ...
    console.debug('_parseData', data.length, from, data.length - from, iStart)
    ...
    if (dataRemaining <= bufRemaining) { // data is smaller/equal rest of buffer
      ...
      if (this._buf.used === this._buf.length) {
        console.debug('buffer filled; handling message (Case #1)')
        try {
          this._handleMessage(this._buf.toString(), this._buf.type)
        } catch (e) {
          console.error('Error while trying to handle message', e)
        }
      }
      ...
      console.debug('buffer filled; handling message (Case #2)')
      try {
        this._handleMessage(this._buf.toString(), this._buf.type)
      } catch (e) {
        console.error('Error while trying to handle message', e)
  ...
  send (msgBody = '', msgType = SerialIO.MESSAGE_TYPE.REQUEST) {
    console.debug(`send length: ${msgBody.length !== undefined ? msgBody.length : '?'}, typeof: ${typeof msgBody}, msgType: ${msgType}`)
    return new Promise((resolve, reject) => {
      if (this.sending) {
